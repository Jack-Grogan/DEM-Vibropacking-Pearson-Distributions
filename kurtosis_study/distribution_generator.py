...
def pearson_pdf(x, mu, sigma, skew, kurt):
    ...
    # Huge help from the "Continuous Univariate Distributions Volume 1" 
...
# Particle diameter array for discretitised continuous distribution
x_bar = np.linspace(mu-x_range, mu+x_range, n_bars)
# Particle diameter array for continuous distribution
x_cont = np.linspace(mu-x_range, mu+x_range, 200)

#------------------------------------------------------------------------------------------
# Plotting PDF for one of sigma, skew and kurt as array
#------------------------------------------------------------------------------------------
...
norm_cont, norm_type_cont =  pearson_pdf(x_cont, mu, sigma, 0, 3)
norm_bar, norm_type_bar =  pearson_pdf(x_bar, mu, sigma, 0, 3)
...
particle_split = [0]*N_curves
for i in range(len(particle_split)):
    particle_split[i] = V_particle_split_bar[i]/sum(V_particle_split_bar[i])
...
for seed in seeds:
    ...
    j2_cylinder_template        = [0]*N_curves
    j2_inface_template          = [0]*N_curves
    j2_simulation_template      = [0]*N_curves
    j2_batch_launch_template    = [0]*N_curves
    ...
    for i in range(N_curves):
        ...
        fractional_data         = {f"F_{int(k)}": round(particle_split[i][k],11) for k in range(len(particle_split))}  
        ...
        simulation_newpath = os.path.join(study_directory, f"kurtosis_{kurt[i]:.3f}")
        if not os.path.exists(simulation_newpath):
            os.makedirs(simulation_newpath)

        batch_launch_newpath = os.path.join(study_directory, f"kurtosis_{kurt[i]:.3f}")
        if not os.path.exists(batch_launch_newpath):
            os.makedirs(batch_launch_newpath)

        mesh_newpath = os.path.join(study_directory, f"kurtosis_{kurt[i]:.3f}", "mesh")
        if not os.path.exists(mesh_newpath):
            os.makedirs(mesh_newpath)

        toml_path = os.path.join(study_directory, f"kurtosis_{kurt[i]:.3f}")
        ...
        ax1.scatter(x_bar, p_bar[i])
        ax1.plot(x_cont, p_cont[i])
        ax1.grid()
        ax1.grid(which='minor', alpha=0.2)
        ax1.minorticks_on()
        ax1.set_title('Pearson Probability Density Function')


        ax2.plot(x_bar, V_particle_split_bar[i])
        ax2.set_title('Constant Volume Distributon')
        ax2.grid()
        ax2.grid(which='minor', alpha=0.2)
        ax2.minorticks_on()

        ax3.plot(particle_radii, particle_split[i])
        ax3.grid()
        ax3.grid(which='minor', alpha=0.2)
        ax3.minorticks_on()
        ax3.set_title('LIGGGHTS Input Distributon')
    ...
    glob_input = os.path.join(study_directory, "kurtosis_*")
