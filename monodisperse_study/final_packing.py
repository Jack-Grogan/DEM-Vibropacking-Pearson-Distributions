...
for seed in seeds:
    ...
    glob_input_directories = os.path.join(seed, "diameter_*")
    ...
    for directory in directories:
        ...
        # np.meshgrid(x, y, z, indexing = ij) take coordinates of the centers along x y and z 
        # mesh 
        # conversion rsin(thetota)
        #xv, yv, zv = np.meshgrid(data.points[:,0], data.points[:,1],data.points[:,2], indexing='ij')
...
df.to_csv('final_packing_results.csv', index = True)