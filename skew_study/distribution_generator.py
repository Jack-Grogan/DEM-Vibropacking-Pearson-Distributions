...
for seed in seeds:
    ...
    for i in range(N_curves):
        ...
        simulation_newpath = os.path.join(study_directory, f"skew_{skew[i]:.3f}")
        if not os.path.exists(simulation_newpath):
            os.makedirs(simulation_newpath)

        batch_launch_newpath = os.path.join(study_directory, f"skew_{skew[i]:.3f}")
        if not os.path.exists(batch_launch_newpath):
            os.makedirs(batch_launch_newpath)

        mesh_newpath = os.path.join(study_directory, f"skew_{skew[i]:.3f}", "mesh")
        if not os.path.exists(mesh_newpath):
            os.makedirs(mesh_newpath)

        toml_path = os.path.join(study_directory, f"skew_{skew[i]:.3f}")
        ...
        ax3.minorticks_on()
        ax3.set_title(f'LIGGGHTS Input Distributon')
    ...
    glob_input = os.path.join(study_directory, "skew_*")
