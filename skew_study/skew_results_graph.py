...
skew_seed = [0]*len(seeds)

for j in range(len(seeds)):
    
    glob_input_directories = os.path.join(seeds[j], "skew_*")
    ...
    skew_seed[j] = directories

skew_seed = np.asarray(skew_seed).T



counted_radii_data_bank = [0]*len(skew_seed)
V_particle_split_bar_dict = [0]*len(skew_seed)
theoretical_dict = [0]*len(skew_seed)
theoretical_dict_values_array  = [0]*len(skew_seed)
particle_radii_input  = [0]*len(skew_seed)
max_percentage_difference_array = [0]*len(skew_seed)
min_percentage_difference_array = [0]*len(skew_seed)
max_difference_array = [0]*len(skew_seed)
min_difference_array = [0]*len(skew_seed)
mean_percentage_difference_array = [0]*len(skew_seed)

for i in positions:
    
    columns = [0]*len(skew_seed[i])
    counted_radii_data = [0]*len(skew_seed[i])
    
    particle_percentage_difference = [0]*len(skew_seed[i])
    particle_percentage_difference_array = [0]*len(skew_seed[i])
    
    particle_difference_array = [0]*len(skew_seed[i])

    particle_radii_array = [0]*len(skew_seed[i])
    ...
    for j in range(len(skew_seed[i])):
    
        columns[j] = skew_seed[i][j].split('\\')[1]
        
        glob_input = os.path.join(skew_seed[i][j], "post", "particles_*")
...
df.rename({'Unnamed: 0': 'skew'}, axis=1, inplace=True)
df_skew = np.asarray(df['skew'])
...
for m in range(len(mean)):
    if mean[m] != mean[-1]:
        for i in range(len(interpolation_number)):
            ...
            interpolation_skew = df_skew[m] + (df_skew[m+1] - df_skew[m])*interpolation_number[i]
            interpolated_skew.append(interpolation_skew)
        ...
        interpolated_skew.append(df_skew[m])
...
for i in range(len(intoplolated_mean)):
    i = i+2
    col_lines_ax5[i-2] = ax5.plot(interpolated_skew[i-2:i], intoplolated_mean[i-2:i], color = colors[i-2], linewidth = 2)
    ax5.fill_between(interpolated_skew[i-2:i], interpolated_topline[i-2:i], interpolated_bottomline[i-2:i], color=colors[i-2], alpha=.05)

# clear reference plots
p, = ax5.plot(df_skew, mean, color = colors[0], linewidth = 0.7, alpha = 0)
f  = ax5.fill_between(df_skew, top_line, bottom_line, color=colors[0], alpha=.0)#, label = '1 Standard Deviation')
...
ellipse_1 = Ellipse((skew[position_1], mean[position_1]), elipse_width_1, height_1, angle=0, linewidth=1.5, fill=False, alpha = 1)
...
ellipse_2 = Ellipse((skew[position_2], mean[position_2]), elipse_width_2, height_2, angle=0, linewidth=1.5, fill=False, alpha = 1)
...
ellipse_3 = Ellipse((skew[position_3], mean[position_3]), elipse_width_3, height_3, angle=0, linewidth=1.5, fill=False, alpha = 1)
...
ellipse_4 = Ellipse((skew[position_4], mean[position_4]), elipse_width_4, height_4, angle=0, linewidth=1.5, fill=False, alpha = 1)
ax5.add_artist(ellipse_4)

# plotting arrows
x_arrow_1 = [(min(particle_radii_input[position_4]) - 6*width_4), 0]

x_arrow_2_ax1 = [(skew[position_1]+ elipse_width_1/2), mean[position_1]]
x_arrow_2_ax2 = [(skew[position_2]+ elipse_width_2/2), mean[position_2]]
x_arrow_2_ax3 = [(skew[position_3]+ elipse_width_3/2), mean[position_3]]
x_arrow_2_ax4 = [(skew[position_4]+ elipse_width_4/2), mean[position_4]]
...
# adding annotations 
ax5.annotate("a)", xy = [-0.86, 0.6426], weight='bold', fontsize = 13)
ax1.annotate(f"b) skew = {skew[position_1]:.2f}", xy = [(min(particle_radii_input[position_4]) - width_1), 4500], weight='bold', fontsize = 13)
ax2.annotate(f"c) skew = {skew[position_2]:.2f}", xy = [(min(particle_radii_input[position_4]) - width_2), 4500], weight='bold', fontsize = 13)
ax3.annotate(f"d) skew = {skew[position_3]:.2f}", xy = [(min(particle_radii_input[position_4]) - width_3), 4500], weight='bold', fontsize = 13)
ax4.annotate(f"e) skew = {skew[position_4]:.2f}", xy = [(min(particle_radii_input[position_4]) - width_4), 4500], weight='bold', fontsize = 13)
...
#saving figure 
plt.savefig('skew_results_plot', bbox_inches="tight", dpi=600)