...
skew = 0
kurt = 3
sigma = np.linspace(1/1000, 1/330, N_curves) #m
...
for seed in seeds:
    ...
    for i in range(N_curves):
        ...
        simulation_newpath = os.path.join(study_directory, f"sigma_{sigma[i]:.8f}")
        if not os.path.exists(simulation_newpath):
            os.makedirs(simulation_newpath)

        batch_launch_newpath = os.path.join(study_directory, f"sigma_{sigma[i]:.8f}")
        if not os.path.exists(batch_launch_newpath):
            os.makedirs(batch_launch_newpath)

        mesh_newpath = os.path.join(study_directory, f"sigma_{sigma[i]:.8f}", "mesh")
        if not os.path.exists(mesh_newpath):
            os.makedirs(mesh_newpath)

        toml_path = os.path.join(study_directory, f"sigma_{sigma[i]:.8f}")
    ...
    glob_input = os.path.join(study_directory, "sigma_*")
