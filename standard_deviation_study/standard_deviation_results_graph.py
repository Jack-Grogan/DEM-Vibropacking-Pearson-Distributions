...
sigma_seed = [0]*len(seeds)

for j in range(len(seeds)):
    
    glob_input_directories = os.path.join(seeds[j], "sigma_*")
    ...
    sigma_seed[j] = directories

sigma_seed = np.asarray(sigma_seed).T



counted_radii_data_bank = [0]*len(sigma_seed)
V_particle_split_bar_dict = [0]*len(sigma_seed)
theoretical_dict = [0]*len(sigma_seed)
theoretical_dict_values_array  = [0]*len(sigma_seed)
particle_radii_input  = [0]*len(sigma_seed)
max_percentage_difference_array = [0]*len(sigma_seed)
min_percentage_difference_array = [0]*len(sigma_seed)
max_difference_array = [0]*len(sigma_seed)
min_difference_array = [0]*len(sigma_seed)
mean_percentage_difference_array = [0]*len(sigma_seed)

for i in positions:
    
    columns = [0]*len(sigma_seed[i])
    counted_radii_data = [0]*len(sigma_seed[i])
    
    particle_percentage_difference = [0]*len(sigma_seed[i])
    particle_percentage_difference_array = [0]*len(sigma_seed[i])
    
    particle_difference_array = [0]*len(sigma_seed[i])

    particle_radii_array = [0]*len(sigma_seed[i])
    ...
    for j in range(len(sigma_seed[i])):
    
        columns[j] = sigma_seed[i][j].split('\\')[1]
        
        glob_input = os.path.join(sigma_seed[i][j], "post", "particles_*")
...
repeats = [0]*len(df_sigma_dimensionless)

for i in range(len(df_sigma_dimensionless)):
...
for i in range(len(repeat_data)):
    for j in df_sigma_dimensionless:
...
for m in range(len(mean)):
    if mean[m] != mean[-1]:
        for i in range(len(interpolation_number)):
            ...
            interpolation_sigma = df_sigma_dimensionless[m] + (df_sigma_dimensionless[m+1] - df_sigma_dimensionless[m])*interpolation_number[i]
            interpolated_sigma.append(interpolation_sigma)
        ...
        interpolated_sigma.append(df_sigma_dimensionless[m])
...
for i in range(len(intoplolated_mean)):
    i = i+2
    col_lines_ax5[i-2] = ax5.plot(interpolated_sigma[i-2:i], intoplolated_mean[i-2:i], color = colors[i-2], linewidth = 2)
    ax5.fill_between(interpolated_sigma[i-2:i], interpolated_topline[i-2:i], interpolated_bottomline[i-2:i], color=colors[i-2], alpha=.05)

# clear reference plots
p, = ax5.plot(df_sigma_dimensionless, mean, color = colors[0], linewidth = 0.7, alpha = 0)
f  = ax5.fill_between(df_sigma_dimensionless, top_line, bottom_line, color=colors[0], alpha=.0)#, label = '1 Standard Deviation')
...
ellipse_1 = Ellipse((df_sigma_dimensionless[position_1], mean[position_1]), elipse_width_1, height_1, angle=0, linewidth=1.5, fill=False, alpha = 1)
...
ellipse_2 = Ellipse((df_sigma_dimensionless[position_2], mean[position_2]), elipse_width_2, height_2, angle=0, linewidth=1.5, fill=False, alpha = 1)
...
ellipse_3 = Ellipse((df_sigma_dimensionless[position_3], mean[position_3]), elipse_width_3, height_3, angle=0, linewidth=1.5, fill=False, alpha = 1)
...
ellipse_4 = Ellipse((df_sigma_dimensionless[position_4], mean[position_4]), elipse_width_4, height_4, angle=0, linewidth=1.5, fill=False, alpha = 1)
ax5.add_artist(ellipse_4)

# plotting arrows
x_arrow_1 = [(min(particle_radii_input[position_4]) - 6.5*width_4), 0]

x_arrow_2_ax1 = [(df_sigma_dimensionless[position_1]+ elipse_width_1/2), mean[position_1]]
x_arrow_2_ax2 = [(df_sigma_dimensionless[position_2]+ elipse_width_2/2), mean[position_2]]
x_arrow_2_ax3 = [(df_sigma_dimensionless[position_3]+ elipse_width_3/2), mean[position_3]]
x_arrow_2_ax4 = [(df_sigma_dimensionless[position_4]+ elipse_width_4/2), mean[position_4]]
...
for l in range(len(particle_radii_input[position_1])):
        ax1.text((particle_radii_input[position_1][l] - width_1/4),(theoretical_dict_values_array[position_1][l] + 600), f"{theoretical_dict_values_array[position_1][l]}", rotation = 90, color = col_lines_ax5[position_1*intopolarion_runs][0].get_color())
for l in range(len(particle_radii_input[position_2])):
        ax2.text((particle_radii_input[position_2][l] - width_2/4),(theoretical_dict_values_array[position_2][l] + 600), f"{theoretical_dict_values_array[position_2][l]}", rotation = 90, color = col_lines_ax5[position_2*intopolarion_runs][0].get_color())
for l in range(len(particle_radii_input[position_3])):
        ax3.text((particle_radii_input[position_3][l] - width_3/4),(theoretical_dict_values_array[position_3][l] + 600), f"{theoretical_dict_values_array[position_3][l]}", rotation = 90, color = col_lines_ax5[position_3*intopolarion_runs][0].get_color())
for l in range(len(particle_radii_input[position_4])):
        ax4.text((particle_radii_input[position_4][l] - width_4/4),(theoretical_dict_values_array[position_4][l] + 600), f"{theoretical_dict_values_array[position_4][l]}", rotation = 90, color = col_lines_ax5[position_4*intopolarion_runs][0].get_color())
...
# adding annotations 
ax1.annotate(f"b) Dimensionless Standard Deviation = {df_sigma_dimensionless[position_1]:.3f}", xy = [(min(particle_radii_input[position_4]) - width_1), 14500], weight='bold', fontsize = 13)
ax2.annotate(f"c) Dimensionless Standard Deviation = {df_sigma_dimensionless[position_2]:.3f}", xy = [(min(particle_radii_input[position_4]) - width_1), 14500], weight='bold', fontsize = 13)
ax3.annotate(f"d) Dimensionless Standard Deviation = {df_sigma_dimensionless[position_3]:.3f}", xy = [(min(particle_radii_input[position_4]) - width_1), 14500], weight='bold', fontsize = 13)
ax4.annotate(f"e) Dimensionless Standard Deviation = {df_sigma_dimensionless[position_4]:.3f}", xy = [(min(particle_radii_input[position_4]) - width_1), 14500], weight='bold', fontsize = 13)
ax5.annotate("a)", xy = [0.1, 0.6347], weight='bold', fontsize = 13)
...
#saving figure 
plt.savefig('sigma_results_plot', bbox_inches="tight", dpi=600)